...
import { Camera, Mic, Volume2, VolumeX, Radio } from "lucide-react";
...
export const Settings = ({ 
    ...
    // Modified initial state for voice activation modes
    const [inputMode, setInputMode] = useState<'voice' | 'push'>('voice');
    ...
    // Click handler for menu propagation
    ...
    const getDevices = async () => {
      try {
        ...
        if (groupedDevices.audiooutput.length) {
          setSelectedAudioOutput(groupedDevices.audiooutput[0].deviceId || "default");
        }
      } catch (error) {
    ...
    const setupPreview = async () => {
      ...
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: { deviceId: selectedCamera === "default" ? undefined : selectedCamera }
        });
  ...
  return (
    ...
    <Card className="w-[600px] bg-gray-900/95 border-gray-600 text-white absolute top-16 right-0 z-[100] shadow-xl" onClick={handleMenuClick}>
      <CardHeader>
                ...
                onOpenChange={(open) => {
                  if (open) {
                    navigator.mediaDevices.enumerateDevices().then(devices => {
                      ...
                      setDevices(prev => ({ ...prev, videoinput: videoDevices }));
                    });
                  }
                }}
              >
                ...
                onOpenChange={(open) => {
                  if (open) {
                    navigator.mediaDevices.enumerateDevices().then(devices => {
                      ...
                      setDevices(prev => ({ ...prev, audioinput: audioDevices }));
                    });
                  }
                }}
              >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <Slider
                  ...
                  max={100}
                  step={1}
                  className={`flex-1 ${(!isMicEnabled || inputMode === 'push') ? "opacity-50" : ""}`}
                />
                ...
                onOpenChange={(open) => {
                  if (open) {
                    navigator.mediaDevices.enumerateDevices().then(devices => {
                      ...
                      setDevices(prev => ({ ...prev, audiooutput: audioDevices }));
                    });
                  }
                }}
              >
